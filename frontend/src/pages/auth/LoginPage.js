...
const LoginPage = () => {
  const { login, isAuthenticated, loading, setUser, setWorkspace, setToken } = useAuth();
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      if (response.ok) {
        ...
        if (data.demo_mode) {
          ...
          try {
            ...
            if (demoResponse.ok) {
              ...
              // Update auth context directly using setters
              setToken(demoData.access_token);
              setUser(demoData.user);
              setWorkspace(demoData.workspace);
              
              toast.success('Demo login successful! (Simulated Google authentication)');
            ...
            toast.error('Demo authentication failed. Please try email/password login.');
          }
        } else {
          // Real OAuth flow
          const { auth_url } = data;
        ...
        toast.error(error.detail || 'Failed to initiate Google login');
      }
    } catch (error) {
      console.error('Google login error:', error);
      toast.error('Failed to connect to Google. Please try again or use email/password login.');
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8">
        ...
        <div className="text-center">
          ...
          <h2 className="text-3xl font-bold text-gray-900">Sign in to VERSATIL</h2>
        ...
        <div className="mt-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
          <h4 className="text-sm font-medium text-blue-900 mb-2">Testing Integration:</h4>
          <p className="text-xs text-blue-700">
            You can test by creating a new account via the "Sign up" link, or use the API directly.
          </p>
        </div>
      </div>
    </div>
